...
$check = $stmt->fetch();
if (!$check['disponible']) {
    header("Location: gestion_voitures.php?nodelete");
    exit();
}
...
if (isset($_POST['idVoiture'])) {
    $req = $bdd->prepare('DELETE FROM voiture WHERE idVoiture = ?');
    $req->execute(array($_POST['idVoiture']));
    $req->closeCursor();

    header("Location: gestion_voitures.php?delete");
    exit();
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <main class="container mt-5">
        <h1 class="display-4 text-uppercase text-center">Suppression de voiture</h1>
        <p>Voulez-vous vraiment supprimer la voiture <?php echo $voiture['marque'] . ' ' . $voiture['modele']; ?> ?</p>
        <form method="post" action="suppression_voiture.php?idVoiture=' . <?php $voiture['idVoiture'] ?> . '">
            <input type="hidden" name="idVoiture" value="<?php echo $voiture['idVoiture']; ?>">
            <input type="submit" class="btn btn-danger" value="Oui"></button>
            <a href="gestion_voitures.php" class="btn btn-dark">Non</a>
        </form>
    </main>
